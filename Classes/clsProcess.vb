...
Public Class clsProcess
    ...
    Private Sub lProcessTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lProcessTimer.Tick
        Try
            ...
            If _Processes.Count <> 0 Then
                For i = 0 To _Processes.Count - 1
                    If _Processes(i).pParam1.Length <> 0 Then
                        n = n + 1
                        Dim lProcessDataArrivalLine As New ProcessInDataDelegate(AddressOf lStatus.lIRCMisc.ProcessDataArrivalLine)
                        Try
                            lBusy = True
            ...
            End If
        Catch ex As Exception
            RaiseEvent ProcessError(ex.Message, "Private Sub lProcessTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lProcessTimer.Tick")
        End Try
    End Sub
    ...
    Public Property Busy() As Boolean
        Get
            Try
                Return lBusy
            Catch ex As Exception
                RaiseEvent ProcessError(ex.Message, "Public Property Busy() As Boolean")
                Return Nothing
            End Try
        End Get
    ...
    Public Sub Add(_StatusIndex As Integer, _Data As String)
        ...
        Catch ex As Exception
            RaiseEvent ProcessError(ex.Message, "Public Sub Add(_StatusIndex As Integer, _Data As String)")
        End Try
    End Sub
