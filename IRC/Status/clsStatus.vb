...
#Region "BOTS"
        Public Sub NickServ_Register(ByVal lStatusIndex As Integer, ByVal lPassword As String, ByVal lEmail As String)
            ...
            End If
            'If Err.Number <> 0 Then Throw ex 'ProcessError(ex.Message, "Public Sub NickServ_Register()")
        End Sub
        Public Sub XLogin(ByVal lStatusIndex As Integer)
            ...
            For i = 1 To lSettings_Services.lServices.sCount
                With lSettings_Services.lServices.sService(i)
                    If Trim(LCase(.sNetwork)) = Trim(LCase(lSettings.lNetworks.nNetwork(NetworkIndex(lStatusIndex)).nDescription)) And .sType = eServiceType.sX Then
                        b = True
                        Exit For
                    End If
                End With
            Next i
            If b = True Or LCase(Trim(lSettings.lNetworks.nNetwork(NetworkIndex(lStatusIndex)).nDescription)) = "undernet" Then
                ...
                End If
            End If
            'If Err.Number <> 0 Then Throw ex 'ProcessError(ex.Message, "Public Sub XLogin()")
        End Sub
        Public Sub NickServLogin(ByVal lStatusIndex As Integer)
            Try
                ...
                For i = 1 To lSettings_Services.lServices.sCount
                    With lSettings_Services.lServices.sService(i)
                        If Trim(LCase(.sNetwork)) = Trim(LCase(lSettings.lNetworks.nNetwork(NetworkIndex(lStatusIndex)).nDescription)) And .sType = Settings.eServiceType.sNickServ Then
                            b = True
                            Exit For
                        End If
                    End With
                ...
                End If
            Catch ex As Exception
                Throw ex 'ProcessError(ex.Message, "Public Sub NickServLogin(ByVal lStatusIndex As Integer)")
            End Try
        End Sub
#End Region
#Region "USER INPUT"
        Public Sub ProcessUserInput(ByVal _StatusIndex As Integer, ByVal _Data As String)
            Try
                ...
                If lStrings.DoLeft(_Data, 1) = "/" Then
                    ...
                    If lStrings.DoLeft(LCase(_Data), 6) = "xlogin" Then
                        Dim f As frmXLogin
                        f = New frmXLogin
                    ...
                    Select Case LCase(Trim(splt(0)))
                        ...
                        Case "echo"
                            ...
                            Exit Sub
                        Case "me"
                            lStrings.ProcessReplaceCommand(_StatusIndex, eCommandTypes.cACTION, lChannels.Name(lChannels.CurrentIndex), Replace(_Data, "me", ""))
                            Exit Sub
                        Case "whowas"
