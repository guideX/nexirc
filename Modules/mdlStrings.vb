...
Module mdlStrings
    ...
    Public Sub LoadStrings()
        'On Error Resume Next
        If (System.IO.File.Exists(lINI.iText)) Then
    ...
    Public Sub DoColor(lData As String, lTextBox As RichTextBox)
        Dim i As Integer, msg As String, lBackColor As Integer = 16, lForeColor As Integer
        If lData.Trim.Length = 0 Then Exit Sub
        If lQuerySettings.qEnableSpamFilter = True Then
            For i = 1 To lQuerySettings.qSpamPhraseCount
                If InStr(lData.ToLower, lQuerySettings.qSpamPhrases(i).ToString.ToLower) <> 0 Then Exit Sub
            Next i
        End If
        ...
        If InStr(lData, "") <> 0 Then
            For i = 0 To lData.Length
                If lData.Length = 0 Then
                    clsScrollToBottom.ScrollToBottom(lTextBox)
                    Exit Sub
                End If
                msg = Left(lData, 1)
                lData = Right(lData, lData.Length - 1)
                If msg = "" Then
                    If IsNumeric(Left(lData, 2)) = True And Not InStr(Left(lData, 2), ",") <> 0 Then
                        lForeColor = CInt(Trim(Left(lData, 2)))
                        lData = Right(lData, lData.Length - 2)
                    ...
                    Else
                        lForeColor = 0
                        lBackColor = 16
                        lTextBox.SelectionColor = ConvertIntToSystemColor(lForeColor, True)
                        lTextBox.SelectionBackColor = ConvertIntToSystemColor(lBackColor, True)
                        If msg <> "" Then
                    ...
                    If Left(lData, 1) = "," Then
                        ...
                        If IsNumeric(Left(lData, 2)) = True And Not InStr(Left(lData, 2), ",") <> 0 Then
                            ...
                            lData = Right(lData, lData.Length - 2)
                        ElseIf IsNumeric(Left(lData, 1)) Then
                            lBackColor = CInt(Left(lData, 1).Trim)
                            lData = Right(lData, lData.Length - 1)
                        End If
                    End If
                Else
